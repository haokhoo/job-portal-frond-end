...
export class EducationUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.educationForm()
    this.auth.getOneEducation(this.route.snapshot.params.id).subscribe(
      education => {
  ...
  updateEducation() {
    this.auth.updateEducation(this.route.snapshot.params.id, this.validateEducation.value).subscribe(
      () => {
        this.auth.showSuccess("Education update Successful");
