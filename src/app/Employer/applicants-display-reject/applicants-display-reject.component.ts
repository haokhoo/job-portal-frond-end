...
@Component({
  selector: 'app-applicants-display-reject',
  templateUrl: './applicants-display-reject.component.html',
  styleUrls: ['./applicants-display-reject.component.scss']
})
export class ApplicantsDisplayRejectComponent implements OnInit {
  eJob: eJob = {
    id: '',
    company_id: '',
    title: '',
  ...
  ngOnInit(): void {
    this.auth.getOneEJobs(this.route.snapshot.params.id).subscribe(
      job => {
        ...
        this.auth.getApplicantsRejected(this.apjob.id, this.apjob.company_id).subscribe(
          applicants => {
